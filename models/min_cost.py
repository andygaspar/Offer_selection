...
class Min_cost:
    ...
    def __init__(self, flights, airlines, slots, new_slot_times, nnb=False):
        self.p = gb.Model()
    ...
    def set_constraints(self):
        for flight in self.flights:
            self.p.addConstr(quicksum(self.x[flight.index, j] for j in self.slots) == 1, name="c1[%s]" % flight)
            self.p.addConstr(
                quicksum(self.x[flight.index, j] for j in self.slots if self.new_slot_times[j] < flight.eta) == 0,
                name="c2[%s]" % flight)
            ...
            for air in self.airlines:
                self.p.addConstr(
                    quicksum(self.x[flight.index, j] * flight.costVect[j] for flight in air.flights for j in
                             self.slots) <= initial_costs(air.flights), name="c4[%s]" % air)

    def set_objective(self):
        self.p.setObjective(
            quicksum(self.x[flight.index, j] * flight.costVect[j] for flight in self.flights for j in self.slots))
    ...
    def solve(self):
        ...
        print("flight  cost  j  airline  2j-eta")
        for j in self.slots:
            for flight in self.flights:
        ...
        total_initial = initial_costs(self.flights)
